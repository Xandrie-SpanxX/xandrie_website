...
export default function Home() {
  ...
  const isReturnPage = typeof window !== 'undefined' && window.location.search.includes('payment_status=completed');
  ...
  useEffect(() => {
    // PayPal script is in index.html, but we need to ensure the container is rendered
    // before the script tries to render the button.
    // Since the script in index.html runs on DOMContentLoaded, and React might render later,
    // we can try to re-trigger the render if needed, or just rely on the script.
    // The script in index.html uses document.addEventListener("DOMContentLoaded", ...)
    // which might have already fired.
    if (window.paypal && window.paypal.HostedButtons) {
      const container = document.getElementById("paypal-container-PUUUKTNVG2QRG");
      if (container && container.innerHTML === "") {
        window.paypal.HostedButtons({
          hostedButtonId: "PUUUKTNVG2QRG"
        }).render("#paypal-container-PUUUKTNVG2QRG");
      }
    }
  }, []);
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            ...
            <a href="https://premium.chat/Goddess_Xandrie_Spanxx" target="_blank" rel="noopener noreferrer" className="block bg-black border border-red-900/50 rounded-lg p-6 hover:border-red-500 transition-all transform hover:scale-105">
              ...
              <p className="text-gray-400 text-sm mt-2">Premium.Chat: <span className="text-red-500 hover:text-red-400">Goddess_Xandrie_Spanxx</span></p>
            </a>
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-gray-900 border border-red-900/30 rounded-lg p-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-gray-300 text-sm">
              ...
              <div>
                <p className="font-semibold text-red-300 mb-2">Financial Domination:</p>
                <p>Findom goddess, paypig training, financial tribute, money slave training, financial control, findom experience, goddess worship findom</p>
              </div>
              <div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div>
              ...
              <RadioGroup 
                ...
                className="space-y-3"
              >
                <div className={`flex items-center space-x-2 w-full bg-gray-900 p-4 rounded-lg border transition-colors cursor-pointer ${ formData.dominanceType === 'findom' ? 'border-red-500 bg-red-900/20' : 'border-red-900/30 hover:border-red-500'}`} onClick={() => setFormData(prev => ({ ...prev, dominanceType: 'findom' }))}>
                ...
                <div className={`flex items-center space-x-2 w-full bg-gray-900 p-4 rounded-lg border transition-colors cursor-pointer ${ formData.dominanceType === 'real_life' ? 'border-red-500 bg-red-900/20' : 'border-red-900/30 hover:border-red-500'}`} onClick={() => setFormData(prev => ({ ...prev, dominanceType: 'real_life' }))}>
                ...
                <div className={`flex items-center space-x-2 w-full bg-gray-900 p-4 rounded-lg border transition-colors cursor-pointer ${ formData.dominanceType === 'hotline' ? 'border-red-500 bg-red-900/20' : 'border-red-900/30 hover:border-red-500'}`} onClick={() => setFormData(prev => ({ ...prev, dominanceType: 'hotline' }))}>
                ...
                <div className={`flex items-center space-x-2 w-full bg-gray-900 p-4 rounded-lg border transition-colors cursor-pointer ${ formData.dominanceType === 'online_chat' ? 'border-red-500 bg-red-900/20' : 'border-red-900/30 hover:border-red-500'}`} onClick={() => setFormData(prev => ({ ...prev, dominanceType: 'online_chat' }))}>
                ...
                <div className={`flex items-center space-x-2 w-full bg-gray-900 p-4 rounded-lg border transition-colors cursor-pointer ${ formData.dominanceType === 'video_chat' ? 'border-red-500 bg-red-900/20' : 'border-red-900/30 hover:border-red-500'}`} onClick={() => setFormData(prev => ({ ...prev, dominanceType: 'video_chat' }))}>
